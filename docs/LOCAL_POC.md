...
The next control-quality test uses the original [Wan2.1-VACE-1.3B](https://huggingface.co/Wan-AI/Wan2.1-VACE-1.3B) and [VACE source](https://github.com/ali-vilab/VACE), both identifying Apache-2.0. `download_vace_benchmark.py` fetches 7.15GB pinned in [the manifest](../config/vace-benchmark.json) and verifies/reuses the cached UMT5 encoder, tokenizer and Wan2.1 VAE. No VACE rendering result is claimed yet. Establish reference-plus-pose quality first, then assess acceleration and streaming independently. [Scope's VACE guide](https://docs.daydream.live/scope/guides/vace) documents experimental streaming control, but its actual [source license](https://github.com/daydreamlive/scope/blob/2aced4ded3513a76cd35f0dbfb42fbfbf5e98ab0/LICENSE.md) is CC BY-NC-SA 4.0 despite MIT package metadata; no Scope code or weights were integrated.
