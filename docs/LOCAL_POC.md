...
Generated-media transport: `experiments/benchmark_cloud_realtime.py --clip <generated audit MP4>` also requires its paired synthetic WAV. The 384×576 clothed-character replay initially delivered audio but no decoded video. Explicitly requesting a keyframe once the subscriber connected fixed this trial: 152 decoded video frames, 298 audio frames, 62ms median / 79ms p95 encode-to-decode delay. Switching the same tracks to a held last frame and silence reached the receiver in 78ms (video) and 156ms (three quiet audio frames), without renegotiation. Setup took 1.266s. Audio is resampled to 48kHz with a matching clock. This is prerecorded generated media, not live inference, perceptual sync or arbitrary-motion acceptance. The keyframe hook is experimental/private aiortc API. Temporary SFU app deletion succeeded; no GPU rental was used and SFU charges were not measured. Six signaling/fixture tests pass. Evidence key: `cloud_realtime_generated_fixture_20260909`.

Live-render transport: `experiments/benchmark_cloud_realtime.py --live-render` reuses the existing GPU frame queue and shared audio clock, without starting another server. Three retained synthetic speech fixtures generated 58/80/49 new lip frames over reviewed body footage and sent them through one Cloudflare connection. First decoded frames arrived 0.702/0.499/0.547s after render submission; all three had zero queue underflows. The receiver decoded 239 video frames (including idle) and 642 audio frames with nonzero amplitude. Setup took 1.250s; both peers remained connected and the temporary app was deleted. This excludes ASR/dialogue/TTS, browser presentation and arbitrary-motion generation. The live app still uses its original transport. Next integration must preserve cancellation and pose continuity rather than replace the working UI prematurely. Local GPU only; electricity/SFU charges unmeasured. Evidence key: `cloud_realtime_live_render_20260909`.

Engine streaming boundary: `CompanionEngine(..., frame_output=...)` accepts an optional context factory `(job_id, chunk_index, audio_path, cancellation_event)` that yields the renderer frame sink. Consumers must discard queued audio/frames after cancellation. The default UI transport is unchanged. A real isolated GPU run emitted 24 frames with speech through this boundary; a second run cancelled after frame index 2, closed the context and preserved memory. Renderer failures and cancellation before rendering are covered by regression tests; 223 Python checks pass. Cold run: 8.488s prepared media, including 7.385s model loading and 0.699s rendering. This is lifecycle validation, not a latency improvement or browser integration. No remote inference or GPU rental; power cost unmeasured. Evidence key: `engine_frame_output_20260909`.

Live-stream cancellation correction: the existing RTC audio track ignored cancellation and continued buffered speech; a regression test reproduced it. Both tracks now check cancellation: audio emits silence and video holds the last frame without consuming queued frames. `experiments/benchmark_cloud_realtime.py --live-render --interrupt` cancelled the third reply after its first decoded frame; five queued frames remained unconsumed and the last five received audio frames were exactly silent after a 0.5s observation window. This is not a measured 0.5s stop latency. Both peers stayed connected without renegotiation, and the temporary SFU app was deleted. The third producer exited with the expected cancellation error. Seven signaling/playback tests pass. Main call UI integration remains pending. Evidence key: `cloud_realtime_live_cancel_20260909`.

Fresh-motion candidate audit: [ShotStream](https://github.com/KlingAIResearch/ShotStream), pinned at `917a0478fe47d71ddcf3269d052cbe3be2ae10e8`, publishes a Wan 1.3B causal implementation, but its documented entry point zeroes first-shot conditioning and returns assembled video rather than a frame stream. Declared default BF16 caches estimate 5.624GiB self-attention plus 2.678GiB context, excluding weights/activations/VAE/text encoder. Its separate interactive pipeline needs compatibility testing. No weights were fetched or upstream code executed. Test identity-preserving conditioning and bounded cache layout before attempting a 16GB run. [MotionStream](https://github.com/alex4727/MotionStream) still lacks released inference; [ARDY](https://github.com/nv-tlabs/ardy) supplies skeletal motion, not photorealistic rendering. Evidence key: `shotstream_source_audit_20260909`.

ShotStream configuration follow-up: the published checkpoint configuration uses six context frames rather than the constructor default of ten. At its declared geometry, estimated video caches total 7.231GiB, not the default-case 8.302GiB. These still exclude models and activations. The separate interactive entry point accepts noise and scheduled prompt switches, with no initial-image argument. Configurations are pinned to Hugging Face revision `5e5e9fce0112e4bec5f5990c0615702509e9e72f`; the 5.676GB inference checkpoint download has started but is not yet verified or integrated.
