...
**Real engine/browser probe:** `--engine-turns` enables bounded command buttons in the isolated transport server with separate test memory. Wave/stop first video: 1.264/0.665s; detected audio: 1.367/0.731s, including TTS. Evidence: `browser_engine_transport_20260909`. Remaining defect: RTC returns to its original idle loop after replies rather than the engine's held/near pose. Fix before switching the main UI.

**Integration still required:** the main UI uses its original transport. `CompanionEngine(..., frame_output=...)` now accepts a context factory `(job_id, chunk_index, audio_path, cancellation_event)` yielding a frame sink. Consumers must discard buffered media on cancellation. Real GPU cancellation after the third frame closed the context without changing memory. Last full suite: 229 Python tests, none skipped. Runtime playback passes an 80-reply queue test, retains at most eight diagnostic rows, and omits benchmark timing markers and the 24-reply cap; this is not a browser long-call test. Shared playback queues and tracks now live in `local_app/rtc.py`; the benchmark retains fixture selection and signaling. This extraction preserves playback behavior and removes the need for the app to import a benchmark server. The experiment queue accepts engine output, copies frames and uses per-turn cancellation; the next turn can resume without closing the session. A frame-queue gap now pauses the audio clock too, including packets crossing the pause boundary; nine targeted RTC tests pass. Browser/perceptual sync under load remains unverified. Historical browser suite: 32 tests.
