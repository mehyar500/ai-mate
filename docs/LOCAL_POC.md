...
**Integration still required:** the main UI uses its original transport. `CompanionEngine(..., frame_output=...)` now accepts a context factory `(job_id, chunk_index, audio_path, cancellation_event)` yielding a frame sink. Consumers must discard buffered media on cancellation. Real GPU cancellation after the third frame closed the context without changing memory. Last full suite: 225 Python tests. The experiment queue accepts engine output, copies frames and uses per-turn cancellation; the next turn can resume without closing the session. A frame-queue gap now pauses the audio clock too, including packets crossing the pause boundary; nine targeted RTC tests pass. Browser/perceptual sync under load remains unverified. Historical browser suite: 32 tests.
