...
In Video call, try **“Come closer.” → “Say hello.” → “Step back.”** The reviewed approach moves from full body to close view; its reverse returns to full body even after intervening conversation. Each pose has a blinking listening loop. These are a limited set of prepared movements, visibly labeled in the UI. Wave uses experimental fresh generation and can fail framing, hand count or direction; arbitrary body commands are not implemented.

Type during a call, or select Enable mic. Text shows messages without ending the active call; Return to call restores the same media elements. “Send me a message saying hello from our call” delivers a separate in-app message with an unread badge. End call releases capture and stops playback. Memory & settings contains facts, notes, diagnostics and Test sound. Camera access is disabled.
