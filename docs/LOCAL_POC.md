...
**Real engine/browser probe:** `--engine-turns` enables bounded command buttons in the isolated transport server with separate test memory. Wave/stop first video: 1.264/0.665s; detected audio: 1.367/0.731s, including TTS. Evidence: `browser_engine_transport_20260909`. RTC now selects the engine's held/near pose; a browser close/stop check retained the close view (first video 2.144/0.945s, audio detection 2.227/1.012s). Stop still freezes a partly open mouth; neutral end frames remain needed. Evidence: `browser_engine_pose_20260909`. Main UI integration remains pending.
