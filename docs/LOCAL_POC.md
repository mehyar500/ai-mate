...
Clone the manifest revision into `.cache/local-poc/RAIN`. Install [isolated imports](../config/rain-benchmark-requirements.txt) with `.venv/Scripts/python.exe -m pip install --no-deps --only-binary=:all: --target .cache/rain-deps -r config/rain-benchmark-requirements.txt`. The app dependencies remain unchanged. Run `download_rain_benchmark.py` for 9.92GB of generator/reference/CLIP assets and `--pose-only` for 351MB of DWPose assets. Primary downloads are in progress at this checkpoint; inspect the active process before restarting.

`benchmark_rain.py --label fresh-controls --prepare-only --frames 64 --side right` prepares a reference-aligned raise/hold/lower sequence using joint angles, with bone-length and frame-bound checks. Both explicit right/left 64-frame controls passed; saved reference/control images were inspected. Maximum numerical arm-length deviation was 5.55e-17 in normalized image coordinates. The first attempt failed on missing matplotlib; isolated dependencies fixed it. One redundant trial was labelled left but actually requested the default right side; it is retained and excluded from left-side evidence. Once model hashes also pass, omit `--prepare-only` for the exclusive GPU renderer test and restart the preview afterward. These are pose controls, not proof of rendered motion, language understanding, speech or complete-call streaming.
