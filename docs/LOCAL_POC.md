...
**Main UI reconnect check (2026-09-09):** Isolated port 8773 accepted a second video call after hangup and the text command “Raise your right hand, then lower it.” Generated artifacts: `generated/local-app/audit/cloud-live-20260909-162640/engine/c227604adcff4d748a5b22fd0010fa86-0.{mp4,wav}`. Browser video reported 384×576, readyState 4, paused=false, muted=false; playback time advanced from 17.429 to 25.994 seconds. Call ended through the UI. This establishes connection recovery and media progression only; physical audibility, visual command correctness, lip sync and fresh-motion latency were not verified by this check. Repaired garbled UTF-8 punctuation in app.js.

**Command media review (2026-09-09):** The reconnect-check clip has 40 frames. Eight sampled frames show the anatomical right hand raising and lowering, with visible hand blur; this remains prepared motion, not fresh continuous control. Its 24kHz mono WAV is 1.281s, RMS −21.70dBFS, peak 0.577. Passing that WAV through the actual RTC Speech track yielded 75 packets / 72,000 samples: the speech samples exactly match the resampled source and the remaining tail is silent. This rules out silence in that source/track path, but does not verify Opus transport, browser speaker output or perceptual lip sync. Evidence beside the clip: `command-review.jpg`, `command-review.json`, `speech-track-review.json`.

**Browser audio reception (2026-09-09):** Main UI now shows aggregate incoming audio packets, signal energy and playback state in Connection & prototype limits, without retaining audio or network identifiers. A live call initially interrupted with the microphone active and showed muted playback. Repeating the hand command with microphone confirmed muted produced 918 received audio packets, positive received audio energy and enabled playback. This verifies browser reception, not physical speaker audibility. Energy is cumulative for the call, not a current-speech detector. No fresh-motion latency was measured.

**Call-start microphone race fixed (2026-09-09):** The pending automatic microphone start is now cancellable while RTC connects. Previously an early microphone toggle could be overridden by the later automatic start. Browser test: start call, immediately cancel microphone request, then send right-hand command; microphone remained muted, 526 incoming audio packets and positive energy were reported with playback enabled. Call ended normally. Twenty existing microphone/call-input tests and JavaScript syntax checks passed. This fixes startup intent preservation, not echo classification or motion generation.

**Short-horizon motion test (2026-09-09):** LTX 2B at 320×480 / 49 frames took 8.287s first execution and 2.513s warm (wall 9.172/3.033s). Reviewing all frames at contact-sheet scale found wrong-hand waving for seed 914 and no clear wave for 2718. Not promoted. Evidence: `short_motion_20260909`. TensorDock advertises RTX 4090 from $0.35/h, A100 SXM4 from $1.80/h, H100 SXM5 from $2.25/h, starting deposit $5 ([source](https://www.tensordock.com/), checked 2026-09-09). These are starting prices, not available US configuration quotes; no rental made. Adult use remains subject to written provider clarification and applicable licenses/policies.

For bounded LongLive GPU trials, `experiments/benchmark_longlive2.py --max-first-chunk-seconds 10` stops after the first decoded chunk if it exceeds ten seconds and retains the observed chunk in the failure record. The optional cutoff includes first-inference overhead; it does not preempt an in-flight GPU kernel. Omit it for an intentional unrestricted timing study.
