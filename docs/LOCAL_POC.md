...
**Real engine/browser probe:** `--engine-turns` enables bounded command buttons in the isolated transport server with separate test memory. Wave/stop first video: 1.264/0.665s; detected audio: 1.367/0.731s, including TTS. Evidence: `browser_engine_transport_20260909`. RTC now selects the engine's held/near pose; a browser close/stop check retained the close view (first video 2.144/0.945s, audio detection 2.227/1.012s). Held replies now append 200ms to restore the source face. One real GPU render produced 30 frames in 0.861s and ended pixel-identically to the inspected closed-mouth source. Browser transition quality remains unverified; evidence: `neutral_tail_20260909`. Evidence: `browser_engine_pose_20260909`. Main UI integration remains pending.

**Integration still required:** the main UI uses its original transport. `CompanionEngine(..., frame_output=...)` now accepts a context factory `(job_id, chunk_index, audio_path, cancellation_event)` yielding a frame sink. Consumers must discard buffered media on cancellation. Real GPU cancellation after the third frame closed the context without changing memory. Last full suite: 230 Python tests, none skipped. Runtime playback passes an 80-reply queue test, retains at most eight diagnostic rows, and omits benchmark timing markers and the 24-reply cap; this is not a browser long-call test. Shared playback queues and tracks now live in `local_app/rtc.py`; the benchmark retains fixture selection and signaling. This extraction preserves playback behavior and removes the need for the app to import a benchmark server. The experiment queue accepts engine output, copies frames and uses per-turn cancellation; the next turn can resume without closing the session. A frame-queue gap now pauses the audio clock too, including packets crossing the pause boundary; nine targeted RTC tests pass. Browser/perceptual sync under load remains unverified. Historical browser suite: 32 tests.
