...
RAIN was tested and rejected for full-body calls: **2.75–2.76 generated FPS**, with distorted anatomy and clothing under both moving and static body controls. Face-only controls improve appearance but remove body guidance. None of these candidates is selected in the app. All frames of their short samples were inspected; speech, live input and browser transport were absent. The next experiment uses the original **Wan2.1-VACE-1.3B** reference-and-pose model. Scope's streaming integration was excluded because its checked-in source license is noncommercial. LOCAL_POC records evidence, setup and limits. No GPU rental or paid rendering API was used for these experiments; electricity cost is unmeasured.
