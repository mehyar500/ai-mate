...
An optional GPU-speech candidate passed 20 commands and six paused-speech cases without functional failures. Across nine spoken replies, end-of-speech median/p95 was **2.04/2.24s**, versus **2.03/2.63s** with CPU speech. The median is essentially unchanged and hosted dialogue varies. The same Kokoro voice synthesized an ordinary sentence in 0.16s versus 0.47s in isolated tests. CPU speech remains selected in the preview while the candidate receives a 30-minute test. The two-second p95 target remains unmet.
