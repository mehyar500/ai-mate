...
Cloudflare Qwen plans replies, CPU Whisper/Kokoro handle recognition and speech, and local MuseTalk produces speech video over prepared body footage. Reviewed approach/return actions maintain position through conversation and interruption. A separately reviewed right-hand wave is implemented and staged; an existing running server needs a restart to load it.

The new wave source took **154.781 seconds** to prepare offline. A subsequent five-turn actual recognition/planning/speech/video browser test started playback in **2.09–2.66 seconds**, with no buffer stalls and completed unmuted audio. The wave used the correct hand and retained full-body framing. Preparation is not live response time.
...
Every decoded frame from the five-turn test was checked for face tracking, dark frames and abrupt pixel changes. Ordered sheets cover all frames; every rendered wave frame was visually reviewed. Correct decoding and low anomaly counts do not certify anatomy, identity or perceptual lip sync. Hand blur, face softness and background deformation remain.
