...
The first GPU-speech candidate failed three replies during a 64-turn soak, despite passing its short tests. The run stopped early; it is not a 30-minute pass. A memory-cleanup revision passed 20 commands with spoken end-of-speech median/p95 of **1.90/2.29s** across nine replies. CPU speech remains selected while the revised GPU candidate receives a fresh sustained test. The two-second p95 target remains unmet, and hosted dialogue varies between runs. [LOCAL_POC](LOCAL_POC.md) records the failure, memory comparison and media review.
...
The GPU-speech call retained 970 analyzed frames, with 40 visually inspected. Two lip-sync estimates passed controls at 0–40ms; a third was inconclusive. No new perceptual acceptance is claimed. The preview's previous restart preserved private memory. **168 Python and 23 Node tests pass.** The retained review page supports playback, exact frame stepping and notes. Physical sound and real mobile/PWA behavior still need device evidence.
