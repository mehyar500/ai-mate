...
Interrupt freezes the displayed frame and reports its timestamp. The engine retains that pose from its own video, so ordinary speech continues there and reviewed approach/return commands can resume their remaining movement. Stale reports and reset races are rejected. Natural speech barge-in and recovery after browser closure remain future work; generation completion still saves text before playback acknowledgment.
