...
An isolated replacement experiment uses **LongLive 2.0 5B**, **UMT5-XXL**, **Wan2.2 VAE** and **MG-LightVAE v2**, pinned in `config/longlive2-benchmark.json`: reference encoding -> changing prompt blocks -> causal diffusion -> cached decoding. Fresh motion runs locally, but right-hand isolation and correct lowering fail. A larger attention history improves the inspected short sequence's continuity; selective FP8 reduces allocation but worsens command following. Speech/playback integration is deferred until renderer quality passes. A separate **RAIN / DWPose / CLIP ViT-L / SD VAE** pose-driven candidate is pinned in `config/rain-benchmark.json`; it is not a validated replacement. No candidate is selected in the engine.
