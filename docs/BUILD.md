...
The local app binds to `127.0.0.1:8765`. Current interaction is **text commands to spoken video replies in one phone-style screen**; microphone and camera are disabled. A Qwen3.5-9B structured planner selects the reply, an allowlisted presentation/scene and verbatim personal-fact excerpts. CPU Kokoro makes speech; local GPU MuseTalk 1.5 generates mouth movement. H.264/AAC fragments stream to the browser while rendering runs, at 20 FPS output. The last two browser samples began after 2.63 and 2.56 seconds with zero buffer waits. These samples are not a latency SLA.

The preferred hybrid split uses an explicitly configured MiniMax API for conversation and reserves the local GPU for graphics. Its adapter exists but no key is configured or hosted latency measured. The local Qwen path is still active. See LOCAL_POC for process variables, source links and qualification gaps.
