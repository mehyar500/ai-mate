...
An isolated replacement experiment uses **LongLive 2.0 5B**, **UMT5-XXL**, **Wan2.2 VAE** and optional **MG-LightVAE v2**, pinned in `config/longlive2-benchmark.json`. The proposed flow is reference encoding -> prompt blocks -> causal diffusion with retained visual state -> cached decoding -> later speech/playback integration. Only decoding and platform compatibility have been tested so far. No new engine route, API key, app dependency or production eligibility is implied.
