...
async function interrupt(){
  ...
  if(rtcPeer){
    ...
    try{
      const result=await api('/api/rtc/stop',{call_id:rtcPeer.callId});
      if(mine!==epoch)return;
...
window.addEventListener("pagehide",()=>{
  ...
  if(rtcPeer){
    ...
    fetch('/api/rtc/close',{method:'POST',headers:{'Content-Type':'application/json','X-Local-Token':token},body:JSON.stringify({call_id:peer.callId}),keepalive:true}).catch(()=>{});
  }
});
async function boot(){
  for(;;){
...
async function closeRtc(){
  ...
  await api('/api/rtc/close',{call_id:peer.callId});
}
async function connectRtc(){
  const peer=new RTCPeerConnection({iceServers:[]});peer.callId=crypto.randomUUID();rtcPeer=peer;
  ...
  try{
    ...
    const answer=await api('/api/rtc/offer',{type:peer.localDescription.type,sdp:peer.localDescription.sdp,call_id:peer.callId});
...
setInterval(async()=>{
  ...
  try{
    const state=await api('/api/rtc/state',{call_id:peer.callId});
